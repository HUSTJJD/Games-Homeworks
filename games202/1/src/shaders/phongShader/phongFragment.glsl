...
float PCF_Filter(sampler2D shadowMap, vec2 uv, float zReceiver,
  ...
  for( int i = 0; i < PCF_NUM_SAMPLES; i ++ ) {
    ...
    if (zReceiver <= depth)
      sum += 1.0;
    depth = unpack(texture2D(shadowMap, uv - poissonDisk[i].yx * filterRadius));
    if (zReceiver <= depth)
      sum += 1.0;
  }
...
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  return PCF_Filter(shadowMap, coords.xy, coords.z, 0.002);
}
