...
public:
    ...
    Triangle(Vector3f _v0, Vector3f _v1, Vector3f _v2, Material *_m = nullptr) : v0(_v0), v1(_v1), v2(_v2), m(_m), Object(EObjectType::Triangle)
    {
        e1 = v1.sub(v0);
        e2 = v2.sub(v0);
    ...
    __device__ bool intersect(const Ray &ray, float &tnear, uint32_t &index) const { return false; }
    ...
    {
        float x = std::sqrt(cuda::get_random_float(thread_id)), y = cuda::get_random_float(thread_id);
...
public:
    MeshTriangle(const std::string &filename, Material *mt = new Material(), Vector3f Trans = Vector3f(0.0, 0.0, 0.0), Vector3f Scale = Vector3f(1.0, 1.0, 1.0)) : Object(EObjectType::MeshTriangle)
    {
        ...
        for (int i = 0; i < mesh.Vertices.size(); i += 3)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                face_vertices[j] = Scale.mul(vert).add(Trans);
                min_vert = Vector3f(std::min(min_vert.x, vert.x),
                                    std::min(min_vert.y, vert.y),
                                    std::min(min_vert.z, vert.z), true);
    ...
    __device__ void getSurfaceProperties(const Vector3f &P, const Vector3f &I, const uint32_t &index, const Vector2f &uv, Vector3f &N, Vector2f &st) const
    {
    ...
    __device__ Vector3f evalDiffuseColor(const Vector2f &st) const
    {
        float scale = 5;
        float pattern =
    ...
    __device__ float getArea() { return area; }
    __device__ bool hasEmit() { return m->hasEmission(); }
